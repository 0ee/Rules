...
if (-1 != $request.url.indexOf('resource/show/tab/v2?') && 0 == body['code']) {
    ...
    body['data']['tab'] = body['data']['tab'].filter(function (item) {
        return item.id != 38247 && item.id != 51079 && item.id != 536 && item.name != '直播' && item.id != 151 && item.id != 165 && item.id != 168 && item.id != 171 && item.id != 136117 && item.name != '追番' && item.name != '推荐' && item.name != '世界杯' 
    });
